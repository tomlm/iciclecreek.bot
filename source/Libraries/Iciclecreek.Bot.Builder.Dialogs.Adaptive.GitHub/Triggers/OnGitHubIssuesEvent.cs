...
{
    /// <summary>
    /// Trigger on github 'issues' webhook event.
    /// </summary>
	public class OnGitHubIssuesEvent : OnGitHubEvent
	{
		/// <summary>
		/// Class identifier.
		/// </summary>
		[JsonProperty("$kind")]
		public new const string Kind = "Iciclecreek.OnGitHubIssuesEvent";

		/// <summary>
		/// Initializes a new instance of the <see cref="OnGitHubIssuesEvent"/> class.
		/// </summary>
		...
		[JsonConstructor]
		public OnGitHubIssuesEvent(string action=null, List<Dialog> actions = null, string condition = null, [CallerFilePath] string callerPath = "", [CallerLineNumber] int callerLine = 0)
		...
		public override Expression GetExpression()
		{
            ...
            else
            {
                actionCondition = Expression.OrExpression(
                    ...
                    Expression.Parse("turn.activity.value.action == 'demilestoned'"),
                    ...
                    Expression.Parse("turn.activity.value.action == 'milestoned'"),
                    Expression.Parse("turn.activity.value.action == 'opened'"),
                    Expression.Parse("turn.activity.value.action == 'pinned'"),
                    Expression.Parse("turn.activity.value.action == 'reopened'"),
                    Expression.Parse("turn.activity.value.action == 'transferred'"),
                    ...
                    Expression.Parse("turn.activity.value.action == 'unpinned'")
                );
            }
            var propertyCondition = Expression.AndExpression(
                Expression.Parse("exists(turn.activity.value.action)"),
                Expression.Parse("exists(turn.activity.value.issue)"),
