...
{
    /// <summary>
    /// Trigger on github 'organization' webhook event.
    /// </summary>
	public class OnGitHubOrganizationEvent : OnGitHubEvent
	{
		/// <summary>
		/// Class identifier.
		/// </summary>
		[JsonProperty("$kind")]
		public new const string Kind = "Iciclecreek.OnGitHubOrganizationEvent";

		/// <summary>
		/// Initializes a new instance of the <see cref="OnGitHubOrganizationEvent"/> class.
		/// </summary>
		...
		[JsonConstructor]
		public OnGitHubOrganizationEvent(string action=null, List<Dialog> actions = null, string condition = null, [CallerFilePath] string callerPath = "", [CallerLineNumber] int callerLine = 0)
		...
		public override Expression GetExpression()
		{
            ...
            else
            {
                actionCondition = Expression.OrExpression(
                    ...
                    Expression.Parse("turn.activity.value.action == 'member_added'"),
                    Expression.Parse("turn.activity.value.action == 'member_invited'"),
                    Expression.Parse("turn.activity.value.action == 'member_removed'"),
                    Expression.Parse("turn.activity.value.action == 'renamed'")
                );
            }
            var propertyCondition = Expression.AndExpression(
                Expression.Parse("exists(turn.activity.value.action)"),
                Expression.Parse("exists(turn.activity.value.membership)"),
