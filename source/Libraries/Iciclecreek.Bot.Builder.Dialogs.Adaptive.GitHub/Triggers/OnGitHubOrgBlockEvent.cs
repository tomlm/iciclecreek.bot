...
namespace Iciclecreek.Bot.Builder.Dialogs.Adaptive.GitHub.Triggers
{
	...
	public class OnGitHubOrgBlockEvent: OnGitHubEvent
	{
		...
		/// </summary>
		[JsonProperty("$kind")]
		public new const string Kind = "Iciclecreek.OnGitHubOrgBlockEvent";

		/// <summary>
		/// Initializes a new instance of the <see cref="OnGitHubOrgBlockEvent"/> class.
		/// </summary>
		...
		[JsonConstructor]
		public OnGitHubOrgBlockEvent(string action=null, List<Dialog> actions = null, string condition = null, [CallerFilePath] string callerPath = "", [CallerLineNumber] int callerLine = 0)
		...
		public override Expression GetExpression()
		{
            ...
            else
            {
                actionCondition = Expression.OrExpression(
                    Expression.Parse("turn.activity.value.action == 'blocked'"),
                    Expression.Parse("turn.activity.value.action == 'unblocked'")
                );
            }
