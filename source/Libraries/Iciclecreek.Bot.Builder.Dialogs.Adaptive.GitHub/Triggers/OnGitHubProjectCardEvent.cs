...
{
    /// <summary>
    /// Trigger on github 'project_card' webhook event.
    /// </summary>
	public class OnGitHubProjectCardEvent : OnGitHubEvent
	{
		/// <summary>
		/// Class identifier.
		/// </summary>
		[JsonProperty("$kind")]
		public new const string Kind = "Iciclecreek.OnGitHubProjectCardEvent";

		/// <summary>
		/// Initializes a new instance of the <see cref="OnGitHubProjectCardEvent"/> class.
		/// </summary>
		...
		[JsonConstructor]
		public OnGitHubProjectCardEvent(string action=null, List<Dialog> actions = null, string condition = null, [CallerFilePath] string callerPath = "", [CallerLineNumber] int callerLine = 0)
		...
		public override Expression GetExpression()
		{
            ...
            else
            {
                actionCondition = Expression.OrExpression(
                    Expression.Parse("turn.activity.value.action == 'converted'"),
                    ...
                    Expression.Parse("turn.activity.value.action == 'moved'")
                );
            }
            var propertyCondition = Expression.AndExpression(
                Expression.Parse("exists(turn.activity.value.action)"),
                Expression.Parse("exists(turn.activity.value.project_card)"),
