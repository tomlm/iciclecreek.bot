...
namespace Iciclecreek.Bot.Builder.Dialogs.Adaptive.GitHub.Triggers
{
	...
	public class OnGitHubRepositoryEvent : OnGitHubEvent
    {
        ...
        /// </summary>
        [JsonProperty("$kind")]
        public new const string Kind = "GitHub.OnRepositoryEvent";
            ...
            if (_expression == null)
            {
                ...
                else
                {
                    actionCondition = Expression.OrExpression(
                        Expression.Parse("turn.activity.value.action == 'archived'"),
                        ...
                        Expression.Parse("turn.activity.value.action == 'privatized'"),
                        Expression.Parse("turn.activity.value.action == 'publicized'"),
                        Expression.Parse("turn.activity.value.action == 'renamed'"),
                        Expression.Parse("turn.activity.value.action == 'transferred'"),
                        Expression.Parse("turn.activity.value.action == 'unarchived'")
                    );
                }
