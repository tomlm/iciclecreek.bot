...
{
    /// <summary>
    /// Action to call GitHubClient.Git.Tree.Get() API.
    /// </summary>
    public class Get : GitHubAction
    {
        ...
        protected override async Task<object> CallGitHubApi(DialogContext dc, Octokit.GitHubClient gitHubClient, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (Owner != null && Name != null && Reference != null)
            {
                ...
                return await gitHubClient.Git.Tree.Get(ownerValue, nameValue, referenceValue).ConfigureAwait(false);
            }
            if (RepositoryId != null && Reference != null)
            {
                var repositoryIdValue = RepositoryId.GetValue(dc);
                var referenceValue = Reference.GetValue(dc);
                return await gitHubClient.Git.Tree.Get((Int64)repositoryIdValue, referenceValue).ConfigureAwait(false);
            }

            throw new ArgumentNullException("Required [reference] arguments missing for GitHubClient.Git.Tree.Get");
        }
    }
}
