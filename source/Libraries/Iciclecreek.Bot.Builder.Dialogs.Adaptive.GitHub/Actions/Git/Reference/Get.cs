...
{
    /// <summary>
    /// Action to call GitHubClient.Git.Reference.Get() API.
    /// </summary>
    public class Get : GitHubAction
    {
        ...
        protected override async Task<object> CallGitHubApi(DialogContext dc, Octokit.GitHubClient gitHubClient, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (Owner != null && Name != null && Reference != null)
            {
                ...
                return await gitHubClient.Git.Reference.Get(ownerValue, nameValue, referenceValue).ConfigureAwait(false);
            }
            if (RepositoryId != null && Reference != null)
            {
                var repositoryIdValue = RepositoryId.GetValue(dc.State);
                var referenceValue = Reference.GetValue(dc.State);
                return await gitHubClient.Git.Reference.Get((Int64)repositoryIdValue, referenceValue).ConfigureAwait(false);
            }

            throw new ArgumentNullException("Required [reference] arguments missing for GitHubClient.Git.Reference.Get");
        }
    }
}
