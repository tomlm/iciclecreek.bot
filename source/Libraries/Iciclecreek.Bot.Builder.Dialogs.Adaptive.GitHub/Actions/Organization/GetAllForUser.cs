...
{
    /// <summary>
    /// Action to call GitHubClient.Organization.GetAllForUser() API.
    /// </summary>
    public class GetAllForUser : GitHubAction
    {
        ...
        /// </summary>
        [JsonProperty("$kind")]
        public const string Kind = "GitHubClient.Organization.GetAllForUser";
        ...
        protected override async Task<object> CallGitHubApi(DialogContext dc, Octokit.GitHubClient gitHubClient, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (User != null && Options != null)
            {
                ...
                return await gitHubClient.Organization.GetAllForUser(userValue, optionsValue).ConfigureAwait(false);
            }
            if (User != null)
            {
                var userValue = User.GetValue(dc);
                return await gitHubClient.Organization.GetAllForUser(userValue).ConfigureAwait(false);
            }

            throw new ArgumentNullException("Required [user] arguments missing for GitHubClient.Organization.GetAllForUser");
        }
    }
}
