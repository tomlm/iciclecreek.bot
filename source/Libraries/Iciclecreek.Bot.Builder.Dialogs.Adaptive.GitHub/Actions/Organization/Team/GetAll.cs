...
{
    /// <summary>
    /// Action to call GitHubClient.Organization.Team.GetAll() API.
    /// </summary>
    public class GetAll : GitHubAction
    {
        ...
        /// </summary>
        [JsonProperty("$kind")]
        public const string Kind = "GitHubClient.Organization.Team.GetAll";
        ...
        protected override async Task<object> CallGitHubApi(DialogContext dc, Octokit.GitHubClient gitHubClient, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (Org != null && Options != null)
            {
                ...
                return await gitHubClient.Organization.Team.GetAll(orgValue, optionsValue).ConfigureAwait(false);
            }
            if (Org != null)
            {
                var orgValue = Org.GetValue(dc);
                return await gitHubClient.Organization.Team.GetAll(orgValue).ConfigureAwait(false);
            }

            throw new ArgumentNullException("Required [org] arguments missing for GitHubClient.Organization.Team.GetAll");
        }
    }
}
