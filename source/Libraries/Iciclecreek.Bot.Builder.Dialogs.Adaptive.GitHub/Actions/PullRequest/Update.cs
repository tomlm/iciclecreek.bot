...
{
    /// <summary>
    /// Action to call GitHubClient.PullRequest.Update() API.
    /// </summary>
    public class Update : GitHubAction
    {
        ...
        /// </summary>
        [JsonProperty("$kind")]
        public const string Kind = "GitHubClient.PullRequest.Update";
        ...
        protected override async Task<object> CallGitHubApi(DialogContext dc, Octokit.GitHubClient gitHubClient, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (Owner != null && Name != null && Number != null && PullRequestUpdate != null)
            {
                ...
                var pullRequestUpdateValue = PullRequestUpdate.GetValue(dc);
                return await gitHubClient.PullRequest.Update(ownerValue, nameValue, (Int32)numberValue, pullRequestUpdateValue).ConfigureAwait(false);
            }
            if (RepositoryId != null && Number != null && PullRequestUpdate != null)
            {
                ...
                var numberValue = Number.GetValue(dc);
                var pullRequestUpdateValue = PullRequestUpdate.GetValue(dc);
                return await gitHubClient.PullRequest.Update((Int64)repositoryIdValue, (Int32)numberValue, pullRequestUpdateValue).ConfigureAwait(false);
            }

            throw new ArgumentNullException("Required [number,pullRequestUpdate] arguments missing for GitHubClient.PullRequest.Update");
        }
    }
}
