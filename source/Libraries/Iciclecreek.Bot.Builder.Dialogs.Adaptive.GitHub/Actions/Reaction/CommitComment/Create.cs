...
{
    /// <summary>
    /// Action to call GitHubClient.Reaction.CommitComment.Create() API.
    /// </summary>
    public class Create : GitHubAction
    {
        ...
        protected override async Task<object> CallGitHubApi(DialogContext dc, Octokit.GitHubClient gitHubClient, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (Owner != null && Name != null && Number != null && Reaction != null)
            {
                ...
                return await gitHubClient.Reaction.CommitComment.Create(ownerValue, nameValue, (Int32)numberValue, reactionValue).ConfigureAwait(false);
            }
            if (RepositoryId != null && Number != null && Reaction != null)
            {
                ...
                var numberValue = Number.GetValue(dc);
                var reactionValue = Reaction.GetValue(dc);
                return await gitHubClient.Reaction.CommitComment.Create((Int64)repositoryIdValue, (Int32)numberValue, reactionValue).ConfigureAwait(false);
            }

            throw new ArgumentNullException("Required [number,reaction] arguments missing for GitHubClient.Reaction.CommitComment.Create");
        }
    }
}
