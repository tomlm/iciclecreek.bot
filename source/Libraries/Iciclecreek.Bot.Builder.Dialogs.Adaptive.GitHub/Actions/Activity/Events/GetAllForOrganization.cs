...
{
    /// <summary>
    /// Action to call GitHubClient.Activity.Events.GetAllForOrganization() API.
    /// </summary>
    public class GetAllForOrganization : GitHubAction
    {
        ...
        /// </summary>
        [JsonProperty("$kind")]
        public const string Kind = "GitHubClient.Activity.Events.GetAllForOrganization";
        ...
        protected override async Task<object> CallGitHubApi(DialogContext dc, Octokit.GitHubClient gitHubClient, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (Organization != null && Options != null)
            {
                ...
                return await gitHubClient.Activity.Events.GetAllForOrganization(organizationValue, optionsValue).ConfigureAwait(false);
            }
            if (Organization != null)
            {
                var organizationValue = Organization.GetValue(dc);
                return await gitHubClient.Activity.Events.GetAllForOrganization(organizationValue).ConfigureAwait(false);
            }

            throw new ArgumentNullException("Required [organization] arguments missing for GitHubClient.Activity.Events.GetAllForOrganization");
        }
    }
}
