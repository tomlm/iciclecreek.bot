...
using Iciclecreek.Bot.Builder.Dialogs.Recognizers.LuLu;
...
namespace LuLuBuild
{
    ...
    class Script : CShell
    {
        public async Task Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine("LuLuBuild folder [--prebuild]");
            ...
            foreach (var file in System.IO.Directory.EnumerateFiles(folder, "*.qna", SearchOption.AllDirectories))
            {
                ...
                if (File.Exists(jsonPath))
                {
                    ...
                    if (prebuild)
                    {
                        ...
                        LuLuceneEngine.CreateCatalog(json, FSDirectory.Open(catalogPath));
                ...
                recognizer["$kind"] = LuLuceneRecognizer.Kind;
